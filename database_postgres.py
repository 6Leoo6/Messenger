...
class DB():
    def __init__(self):
        ...
        conn.cursor().execute("CREATE TABLE IF NOT EXISTS main (id text, data json, img bytea)")
        conn.commit()
    ...
    def saveMessage(self, data: dict):
        ...
        if latestChat == 0 or json.loads(cur.fetchone()[0])['n'] == self.MAX_MESSAGES:
            cur.execute(f'INSERT INTO {chatId} VALUES (%s, %s)', [latestChat+1, json.dumps({'i': latestChat+1, 'n': 0, 'msgs': {}})])
            latestChat += 1
        
        cur.execute(f'SELECT data FROM {chatId} WHERE number=%s', [latestChat])
        existing = json.loads(cur.fetchone()[0])
